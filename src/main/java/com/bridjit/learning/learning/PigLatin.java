 ...
 * It obeys a few simple rules (below), but when it's spoken quickly it's really difficult for non-children (and non-native speakers) 
 * to understand.
 *
...
public class PigLatin {
	...
	public static String toPigLatin(String word) {
		Optional<String> rule1 = Optional.of(word).filter((w) -> w.toLowerCase().matches("^(a|e|y|u|i|o|xr|yt)")).map((w) -> w.concat("ay"));
		Optional<String> rule2 = Optional.of(word).filter((w) -> w.toLowerCase().matches("^(a|e|y|u|i|o|xr|yt)")).map((w) -> w.concat("ay"));
		return "";
	}
}
